...
p = psutil.Process()
p.cpu_affinity(list(random.sample(range(1, count), 8)))
...
if __name__ == '__main__':
    for bit in [128,]:
        for n_class in [10,]:
            if n_class == 100 and bit in [8,16,32,64]:
                continue
            initWithCSQ = True
            # if bit == 48:
            #     initWithCSQ = False
            ...
            if bit > 64:
                # 超过这个的时候会导致内存开销极大，并且几乎无法训练，因此直接使用CSQ_init的结果
                best_B = init_hash(n_class, bit) # random initialization
                np.save(f'./tmp_file/MDSH/init_{n_class}_{bit}.npy', best_B)
            else:
                # 初始评价指标
